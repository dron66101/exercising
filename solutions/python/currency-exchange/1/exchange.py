...
def exchangeable_value(budget:float, exchange_rate:float, spread:int, denomination:int) -> int:
    ...
    """
    exchangedBudget = exchange_money(budget, exchange_rate+(exchange_rate*(spread/100)))
    denominationActions = get_value_of_bills(get_number_of_bills(exchangedBudget, denomination), denomination)

    return int(denominationActions)
