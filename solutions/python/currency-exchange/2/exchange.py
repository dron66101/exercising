...
def exchangeable_value(budget:float, exchange_rate:float, spread:int, denomination:int) -> int:
    ...
    """
    exchanged_budget = exchange_money(budget, exchange_rate+(exchange_rate*(spread/100)))
    denomination_actions = get_value_of_bills(get_number_of_bills(exchanged_budget, denomination), denomination)

    return int(denomination_actions)
